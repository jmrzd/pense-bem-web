...
const SONIC_PROGRAM_IDS = new Set([
  '131',
  '132',
  '133',
  '134',
...
export function ProgramSelection() {
  ...
  // Mostra somente os 5 programas do livro
  // Sonic e Tails em Alta Velocidade.
  const sonicPrograms = programs.filter((program) =>
    SONIC_PROGRAM_IDS.has(program.id),
  )
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-10">
      ...
      >
        ...
        <h1 className="mt-2 font-hero text-4xl sm:text-5xl">
          ESCOLHA SEU PROGRAMA
        </h1>

        <p className="mt-3 text-ink-soft">
          Sonic e Tails em Alta Velocidade — cada programa tem
          30 perguntas e até 90 pontos disponíveis.
        </p>
      </motion.div>
      ...
      {!error && !loading && (
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {sonicPrograms.map((program, index) => {
            const session = player
              ? getSession(program.id, player.id)
              : null

            const inProgress =
              session !== null &&
              session.currentIndex + session.answers.length > 0
            ...
            return (
              ...
              >
                <Panel className="relative h-full">
                  ...
                  <span className="panel-sm absolute -right-3 -top-3 flex h-11 w-11 rotate-6 items-center justify-center rounded-full bg-paper font-hero text-xs">
                    {program.id}
                  </span>

                  <div
                    className={`flex h-14 w-14 items-center justify-center rounded-xl border-[2.5px] border-ink ${
                      ACCENT_CLASSES[program.accent]
                    }`}
                  >
                    <Icon
                      name={
                        PROGRAM_ICONS[program.id] ?? 'star'
                      }
                      size={28}
                      strokeWidth={2}
                    />
                  ...
                  {inProgress ? (
                    <div className="panel-sm mt-4 inline-flex items-center gap-1.5 rounded-lg bg-cobalt px-4 py-2 text-sm font-bold text-chip-light">
                      ...
                      Continuar (pergunta{' '}
                      {session!.currentIndex + 1}/30)
                    </div>
                  ) : (
                    <div
                      className={`panel-sm mt-4 inline-flex items-center gap-1.5 rounded-lg px-4 py-2 text-sm font-bold ${
                        ACCENT_CLASSES[program.accent]
                      }`}
                    >
                      Jogar agora →
